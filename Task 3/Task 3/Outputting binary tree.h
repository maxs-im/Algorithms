	...
	default_ios(ios_type)
{
	...
	if(default_ios == DEFAULT) fout.open(path_to_file);
	else fout.open(path_to_file, std::ios_base::app);
							 ...
							 unsigned short height, const unsigned short *border)
{
	...
	if(height >= border[1]) {
		...
		fout<< std::setfill(FILL_KEY)<< std::setw(place_for_number)<< &curr->data;
